...
def player_order(data):
    ...
    if data[1] == Constant.ID_2 and data[0] == Constant.SELECT_FIRST_PLAYER:
        isActivePlayer = True
        ready = True
    elif data[1] != Constant.ID_2 and data[0] == Constant.SELECT_FIRST_PLAYER:
        isActivePlayer = False
        ready = True
    else:
        ready = False
...
pygame.display.set_caption("Billard - Client 2")
    ...
    while not ready:
        ...
        for event in pygame.event.get():
            if event.type == pygame.MOUSEBUTTONDOWN and not ready_send:
                ...
                ready_send = True
                s.send(pickle.dumps([Constant.PLAYER_READY, Constant.ID_2]))
                time.sleep(0.1)
                break
    # Ready to start the game
    print("ready to start the game")
    s.send(pickle.dumps([Constant.ALL_PLAYER_READY, Constant.ID_2]))
    ...
    while game_running:
        if isActivePlayer:
            ...
            while not shoot:
                ...
                clock.tick(60)
            dir = [x2-x1, y2-y1]
            s.send(pickle.dumps([Constant.GAME_DATA, Constant.ID_2, dir, score, number_of_ball]))
            Physics.shoot(screen, dir[0], dir[1], ball, [Constant.ID_2, score, number_of_ball])
            isActivePlayer = False
